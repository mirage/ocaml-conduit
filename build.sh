...
HAVE_MIRAGE=`ocamlfind query mirage 2>/dev/null || true`
...
if [ "$HAVE_LWT" != "" ]; then
  ...
  echo Lwt_conduit_resolver > lib/conduit-lwt.mllib
  add_target "conduit-lwt"
  LWT_REQUIRES="lwt"
  LWT_UNIX_REQUIRES="lwt.unix ipaddr.unix uri.services"
  LWT_SYNTAX=lwt.syntax

  echo Lwt_unix_conduit > lib/conduit-lwt-unix.mllib
  echo Lwt_unix_net >> lib/conduit-lwt-unix.mllib
  echo Lwt_unix_resolver >> lib/conduit-lwt-unix.mllib
  ...
  if [ "$HAVE_LWT_SSL" != "" ]; then
    ...
    echo Lwt_unix_net_ssl >> lib/conduit-lwt-unix.mllib
  fi

  if [ "$HAVE_MIRAGE" != "" ]; then
    echo "Building with Mirage support."
    echo 'true: define(HAVE_MIRAGE)' >> _tags
    echo Mirage_conduit > lib/conduit-lwt-mirage.mllib
    echo Mirage_resolver >> lib/conduit-lwt-mirage.mllib
