...
module Memory_flow0 = struct
  ...
  let recv flow buf =
    ...
    if len = 0 then ( flow.c <- true ; Ok `End_of_flow )
    else
...
let test_input_string =
  ...
  Alcotest.(check (result send error)) "res2" res2 (Error (`Msg Memory_flow0.closed_by_peer)) ;
;;
...
module Memory_flow1 = struct
  ...
  let recv flow buf =
    ...
    List.iter (fun x ->
        if !acc < max
        then ( let len = min (max - !acc) (String.length x) in
               Bytes.blit_string x 0 buf !acc len
             ; acc := !acc + len))
      flow.i ;
    flow.i <- shift !acc flow.i ;
    if !acc = 0 then ( flow.c <- true ; Ok `End_of_flow )
    else Ok (`Input !acc)
  ...
  let send flow str =
    if flow.c then Error `Closed
      ...
      List.iter (fun x ->
          ...
          then ( let len = max 0 (Bytes.length x + (!pos + !acc)) in
...
let test_input_strings =
  ...
  let open Rresult in
  let flow = Conduit.connect ([ ""; "123"; "45"; "6789"; "0" ], [ Bytes.empty ]) memory1 in
  Alcotest.(check bool) "connect" (R.is_ok flow) true ;
  let flow = R.get_ok flow in
  ...
  Alcotest.(check (result send error)) "res4" res4 (Error (`Msg Memory_flow1.closed_by_peer)) ;
;;
...
let test_output_strings =
  ...
  Alcotest.(check (result send error)) "res3" res3 (Error (`Msg Memory_flow1.closed_by_peer)) ;
  Alcotest.(check string) "bufs" (String.concat "" (List.map Bytes.to_string bufs)) "Hello World!" ;
;;

let () =
  Alcotest.run "flow"
    [ "memory", [ test_input_string
                ; test_output_string
                ; test_input_strings
                ; test_output_strings ] ]
