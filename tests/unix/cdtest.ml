...
let perform () =
  ...
  let t =
    Array.init limit (fun _ -> client_test_wait 10. wait)
    |> Array.to_list
    |> Lwt.join
  in
  Lwt.catch
    (fun () ->
      ...
      let rec wait_all_conn () =
        ...
        if !active < limit then wait_all_conn () else Lwt.return_unit
      in
      wait_all_conn () >>= fun () ->
      print_endline "Waiting for error";
      (* use a lower timeout here, these should fail immediately *)
      Array.init (2 * limit) client_test |> Array.to_list |> Lwt.pick
      >>= fun () ->
...
let () =
  ...
  Lwt_main.run
    (Lwt_unix.with_timeout 60. (fun () -> Lwt_unix.handle_unix_error perform ()));
  print_endline "OK"
